...
export function fitTreeHmm(profiles: readonly TreeEmissionProfile[], options: TreeHmmOptions): TreeHmmResult {
  ...
  const verificationIndexes = subsetSearch === undefined
    ...
    : Array.from(new Map([
      ...
      subsetSearch.hypotheses.find((hypothesis) => hypothesis.key === subsetSearch.nullKey),
  ...
  const exactFits = verificationIndexes.map((indexes, verificationIndex) => {
    if (indexes.length === 1 && indexes[0] === 0) return nullFit;
    return fitSubset(profiles, indexes, options, (completed, total, fit) => options.onProgress?.(
  ...
  const exactSelectedKey = current.indexes.join(",");
  ...
  while (searchMode !== "fixed" && current.indexes.length > 1) {
    ...
    for (let index = 0; index < removals.length; index += 1) {
      ...
      exactByKey.set(subset.join(","), fit);
  ...
  const auditedSubsetSearch = subsetSearch === undefined ? undefined : {
    ...
    exactSelectedKey,
    finalSelectedKey: current.indexes.join(","),
  };
