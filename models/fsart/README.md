...
8. The consensus cuts define atomic segments. FastTree 2.1.11 bioWASM fits GTR+Gamma trees to the global alignment, every atomic segment, every contiguous pair of segments, and every contiguous triplet—never larger combinations. Every resolved full-tree fit is retained independently, including equal topologies with different source-fitted branch lengths or Gamma shapes. The global fit supplies one shared GTR rate/frequency model; an internal frozen-tree likelihood scorer then evaluates **every alignment site** without re-estimating the tree or mixing sites from outside its source window into parameter fitting.
...
12. Final Viterbi runs are selectable in the result studio. Two selections produce a mirrored tip-to-tip tanglegram. Three or more produce same-direction linked trees beneath the topology-posterior/Viterbi alignment track. Display-only rerooting and child flips jointly minimize matching-taxon crossings with a mild ladderization preference; they never alter topology or branch lengths. All linked-tree views export as SVG.
