...
import { fitLinkedNetworkLikelihood } from "./likelihood-refinement.js";
import { searchSwitchingNetwork } from "./network-search.js";
import { searchRootedTreePath } from "./path-search.js";
import type { JemsprAnalysisResult, JemsprLikelihoodResult, JemsprOptions } from "./types.js";
...
export async function analyzeJemspr(alignmentText: string, options: JemsprOptions = {}): Promise<JemsprAnalysisResult> {
  ...
  let likelihood: JemsprLikelihoodResult;
  if (!(options.linkedLikelihood ?? true)) likelihood = { status: "skipped", reason: "Linked branch-length ML was disabled in the advanced controls." };
  else if (options.gtrModel === undefined) likelihood = { status: "skipped", reason: "The alignment-viewer FastTree runtime did not provide a valid global GTR matrix; topology/network inference remains complete." };
  else likelihood = fitLinkedNetworkLikelihood(alignment, network.network, network.masks, network.maskPath, options);
  ...
  if (likelihood.status === "skipped") warnings.push(likelihood.reason);
  else if (likelihood.nonidentifiableGroups.length > 0) warnings.push(`${likelihood.nonidentifiableGroups.length} atomic-edge group${likelihood.nonidentifiableGroups.length === 1 ? " is" : "s are"} identifiable only through summed displayed-branch lengths; the result JSON lists these exact duplicate design columns.`);
  ...
  const resultBase = {
    ...
    timings: {
      ...
      linkedLikelihoodMs: likelihoodMs,
  ...
  const result: JemsprAnalysisResult = {
    ...
    networkJson: JSON.stringify({ schemaVersion: 1, method: "jemspr", taxaNames: alignment.names, switchingNetwork: network.serializableNetwork, masterTree: network.public.masterTree, linkedLikelihood: likelihood, templates: network.public.templates, occurrences: network.public.occurrences, maskRuns: network.public.runs, temporal: network.public.temporal, diagnostics: resultBase.diagnostics }, null, 2),
  };
