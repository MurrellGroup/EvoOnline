...
export const jemsprManifest: ModelManifest = {
  ...
  parameters: [
    ...
    { id: "overlapCap", label: "Maximum concurrent events", description: "All active-event masks through this overlap depth are decoded exactly. Increase if the result touches the cap.", type: "integer", default: 2, minimum: 0, maximum: 6, step: 1 },
    ...
    { id: "pathBreakpointPenalty", label: "Path breakpoint penalty", description: "Zero selects log2(alignment length + 1).", type: "number", default: 0, minimum: 0, maximum: 100, step: 0.1, advanced: true },
    { id: "pathEndpointPenalty", label: "Path rSPR endpoint penalty", description: "Zero selects a taxon-count default. Multi-rSPR changes at one boundary pay this per graph edge.", type: "number", default: 0, minimum: 0, maximum: 100, step: 0.1, advanced: true },
    { id: "pathSpanPenalty", label: "Path master-distance span penalty", description: "Zero selects the inverse-window default. This term identifies a compact latent master without requiring it to occur locally.", type: "number", default: 0, minimum: 0, maximum: 1, step: 0.0005, advanced: true },
    { id: "networkBeamWidth", label: "Switching-network beam", description: "Diverse exact-scored networks retained after each reticulation addition.", type: "integer", default: 6, minimum: 1, maximum: 32, step: 1, advanced: true },
    { id: "eventPoolSize", label: "Residual event-template pool", description: "Master-relative rooted-SPR moves available to the network compiler.", type: "integer", default: 14, minimum: 1, maximum: 48, step: 1, advanced: true },
    { id: "eventOpenPenalty", label: "Event opening penalty", description: "Cost for starting one persistent reticulation-template occurrence.", type: "number", default: 4, minimum: 0, maximum: 100, step: 0.1, advanced: true },
    ...
    { id: "networkBreakpointPenalty", label: "Network breakpoint-coordinate penalty", description: "Charged once at a genomic coordinate regardless of how many event bits change there.", type: "number", default: 4, minimum: 0, maximum: 100, step: 0.1, advanced: true },
    { id: "eventSpanPenalty", label: "Active-event span penalty", description: "Per-nucleotide cost for each simultaneously active reticulation template.", type: "number", default: 0.004, minimum: 0, maximum: 1, step: 0.0005, advanced: true },
    { id: "reticulationPenalty", label: "Network structural penalty", description: "Cost per retained reticulation template, including unused templates during beam search.", type: "number", default: 3, minimum: 0, maximum: 100, step: 0.1, advanced: true },
