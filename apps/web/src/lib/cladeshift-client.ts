...
export class CladeShiftClient {
  private worker: Worker | undefined;
  private rejectActive: ((error: Error) => void) | undefined;

  private createWorker(): Worker {
    return new Worker(new URL("../workers/cladeshift.worker.ts", import.meta.url), { type: "module" });
  }

  run(alignment: string, tree: string, parameters: ParameterValues, onProgress: (progress: RunProgress) => void): Promise<CladeShiftRunResult> {
    this.cancel();
    ...
    return new Promise((resolve, reject) => {
      ...
      worker.onmessage = (event: MessageEvent<CladeShiftWorkerResponse>) => {
      ...
      worker.onerror = (event) => { this.finish(); reject(new Error(event.message || "CladeShift worker failed.")); };
