...
export function FsartInferenceExplorer({ result }: { readonly result: FsartAnalysisResult }) {
  ...
  return <>
    ...
    {mode === "conservative" && <p className="figure-note"><strong>{manuallyPolished ? "Saved manual alternative." : "Original reconstruction retained unchanged."}</strong> {manuallyPolished ? "The requested cached topology subset was fit exactly, then alternated with Viterbi-assigned tree re-estimation and breakpoint polishing. The initial automatic search remains available in the hypothesis audit above." : "This is rapid subset screening, bounded exact-finalist verification, exact rate marginalization, and bounded Viterbi/tree refitting."}</p>}
