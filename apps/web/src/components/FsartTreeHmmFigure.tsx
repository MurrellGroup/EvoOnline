...
import type { TreeHmmResult } from "@phylo-workbench/model-fsart/browser-source";
import { downloadSvg } from "../lib/svg-export.js";

const FONT = "Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif";

export function FsartTreeHmmFigure({ result }: { readonly result: TreeHmmResult }) {
  const titleId = useId();
  const svgRef = useRef<SVGSVGElement>(null);
  const [title, setTitle] = useState("Topology-HMM posterior along the alignment");
  ...
  return <article className="figure-card fsart-tree-hmm-card">
    ...
    <div className="tree-figure-controls fsart-figure-controls">
      ...
      <label className="toggle"><input type="checkbox" checked={showIntervals} onChange={(event) => setShowIntervals(event.target.checked)} /><span>Switch intervals</span></label>
    </div>
