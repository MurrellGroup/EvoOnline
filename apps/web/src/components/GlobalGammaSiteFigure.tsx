...
function metricName(metric: Metric, branchName: string | undefined): string {
  return {
    capped: "log₁₀ all-branches capped evidence ratio",
    ...
    "branch-local": `${branchName ?? "Selected branch"}: local cap log evidence`,
  }[metric];
}
...
export function GlobalGammaSiteFigure({
  ...
  const [title, setTitle] = useState("Global-Gamma evidence across codon sites");
  ...
  const range = useMemo(() => {
    ...
    if (diverging) {
      ...
      const robust = absolute[Math.min(absolute.length - 1, Math.floor(absolute.length * 0.98))] ?? 1;
      const bound = Math.max(1e-6, robust);
    ...
    const maximum = effectiveMetric === "support" || effectiveMetric === "maximum" || effectiveMetric === "branch-tail"
      ? 1
      : Math.max(1e-6, sorted[Math.min(sorted.length - 1, Math.floor(sorted.length * 0.98))] ?? 1);
    return { minimum: 0, maximum };
  }, [diverging, effectiveMetric, values]);
  const left = 74;
  const right = 24;
  const top = 54;
  const bottom = 58;
  ...
  return <article className="figure-card global-gamma-site-card">
    ...
    <div className="tree-figure-controls global-gamma-site-controls">
      ...
      <label><span>Site metric</span><select value={metric} onChange={(event) => setMetric(event.target.value as Metric)}>
        <option value="capped">All-branches capped evidence</option><option value="support">Equal-prior conditional support</option>
    ...
    <div className="figure-scroll" tabIndex={0}><svg ref={svgRef} viewBox={`0 0 ${width} ${height}`} width={width} height={height} role="img" aria-labelledby={titleId} style={{ display: "block", minWidth: `${width}px`, background: "#fff", fontFamily: FONT }}>
      ...
      {values.map((value, index) => {
        ...
        return <rect key={site} x={x(site) - Math.max(1, columnWidth * 0.38)} y={barY} width={Math.max(1, columnWidth * 0.76)} height={barHeight} fill={fill} opacity={site === selectedSite ? 1 : 0.72} onClick={() => onSelectSite(site)} style={{ cursor: "pointer" }}><title>{`Codon ${site} · ${label}: ${value.toPrecision(5)}`}</title></rect>;
      })}
