...
export function GlobalGammaResultsView({ result, threshold, alignment }: { readonly result: GlobalGammaRunResult; readonly threshold: number; readonly alignment: string }) {
  ...
  return <section className="results" aria-labelledby="global-gamma-results-heading">
    <div className="section-heading section-heading--results"><div><p className="eyebrow">Analysis complete · exploratory model</p><h2 id="global-gamma-results-heading">Global Gamma branch–site scan</h2><p>Globally fitted Gamma(ω) random effects across branch–site cells, with a mean-one Gamma(α) mixture across sites.</p></div><div className="result-downloads"><button type="button" className="button button--primary" onClick={() => downloadCsv(result.branchCsv, "global-gamma-branches.csv")}>Branch CSV</button><button type="button" className="button button--secondary" onClick={() => downloadCsv(result.siteCsv, "global-gamma-sites.csv")}>Site CSV</button></div></div>
    <p className="method-note"><strong>Likelihood hierarchy:</strong> for each α category, the same site-wise α is used on every edge while each edge independently integrates the fitted Gamma(ω) categories; complete-tree site likelihoods are then averaged over α. Site evidence caps every edge, whereas branch evidence caps exactly one edge using its two-sided local blanket. The activation empirical BF integrates <em>λ</em> under Beta({result.diagnostics.activationPriorAlpha.toPrecision(3)}, {result.diagnostics.activationPriorBeta.toPrecision(3)}). These are plug-in conditional/empirical-Bayes quantities, not calibrated LRT p-values.</p>
    <div className="result-stats">
      ...
      <div><span>Capped-edge ER ≥ 10</span><strong className="positive-text">{cappedSupported.toLocaleString()}</strong></div>
    ...
    <details className="result-panel" open><summary><span>Site evidence table</span><small>{visibleSites.length.toLocaleString()} rows shown{result.sites.length > 500 && siteFilter === "all" ? " · first 500" : ""}</small></summary><div className="result-panel__body">
      <div className="selection-visibility bsrel-table-controls"><label><span>Site filter</span><select value={siteFilter} onChange={(event) => setSiteFilter(event.target.value as typeof siteFilter)}><option value="all">All sites</option><option value="supported">All-branches capped ER ≥ 10</option><option value="tail">Max branch posterior ≥ threshold</option></select></label><label><span>Tail threshold</span><CommittedNumberInput value={posteriorThreshold} onCommit={setPosteriorThreshold} min={0.5} max={0.999} step={0.01} integer={false} /></label><label><span>Selected codon</span><CommittedNumberInput value={selectedSite} onCommit={commitSite} min={1} max={result.posterior.siteCount} /></label></div>
      <div className="result-table-wrap"><table className="result-table"><thead><tr><th>Codon</th><th>All-branches capped log ER</th><th>Evidence ratio</th><th>Conditional support</th><th>E[positive branches]</th><th>Max branch posterior</th></tr></thead><tbody>{visibleSites.map((site) => <tr key={site.site} className={selectedSite === site.site ? "is-selected" : undefined} onClick={() => setSelectedSite(site.site)}><td><strong>{site.site}</strong></td><td className={site.cappedLogEvidence >= evidenceCutoff ? "positive-text" : undefined}>{number(site.cappedLogEvidence)}</td><td>{number(site.cappedEvidenceRatio)}</td><td>{probability(site.conditionalSupport)}</td><td>{number(site.expectedPositiveBranches)}</td><td className={site.maximumBranchPosterior >= posteriorThreshold ? "positive-text" : undefined}>{probability(site.maximumBranchPosterior)}</td></tr>)}</tbody></table></div>
    </div></details>

    <ReferenceResultMap modelName="Global Gamma" alignmentText={alignment} evidenceSites={referenceEvidence} hypotheses={GLOBAL_GAMMA_REFERENCE_HYPOTHESES} initialThreshold={posteriorThreshold} />
  </section>;
}
