...
import { FsartSprReconstructionFigure } from "./FsartSprReconstructionFigure.js";
...
export function FsartResultsView({ result }: { readonly result: FsartAnalysisResult }) {
  ...
  return <section className="results" aria-labelledby="fsart-results-heading">
    ...
    <p className="method-note"><strong>What FSART asks:</strong> does phylogenetic support change along the nucleotide alignment, and can those changes be represented by a compact series of subtree-prune-regraft events? A pair-covered informative-triplet scan generates an uncorrected evidence distribution without a multiple-comparisons admission gate. Consensus boundaries generate a diverse FastTree proposal family. EvoOnline then runs both a cached-likelihood topology HMM and a separate parsimony-first search that jointly revises an unknown master tree while allowing every local tree to be an unrestricted composition of SPR edits.</p>
    ...
    <div className="result-stats">
      ...
      {result.sprReconstruction !== undefined && <div><span>SPR history</span><strong>{result.sprReconstruction.status === "complete" ? `${result.sprReconstruction.events.length} boundaries` : result.sprReconstruction.status}</strong><small>{result.sprReconstruction.status === "complete" ? `${result.sprReconstruction.events.reduce((total, event) => total + event.sprDistance, 0)} composed edits` : result.sprReconstruction.message}</small></div>}
    ...
    {result.sprReconstruction !== undefined && <details className="result-panel" open><summary><span>Unknown-master, unrestricted SPR reconstruction</span><small>{result.sprReconstruction.status === "complete" ? `${result.sprReconstruction.runs.length} runs · ${result.sprReconstruction.events.reduce((total, event) => total + event.sprDistance, 0)} explicit edits · master ${result.sprReconstruction.masterStateId}` : result.sprReconstruction.message}</small></summary><div className="result-panel__body"><FsartSprReconstructionFigure result={result.sprReconstruction} /></div></details>}
    ...
    {result.discordantClades.length > 0 && <details className="result-panel"><summary><span>Exploratory participating-subtree candidates</span><small>Legacy adjacent-run split flags; the explicit event tape above is the reconstruction</small></summary><div className="result-panel__body"><p className="figure-note">This legacy table is retained as a topology-difference cross-check. It is not substituted for the explicit, potentially multi-edit SPR scripts reported above.</p><div className="result-table-wrap"><table className="result-table"><thead><tr><th>Segments</th><th>Change</th><th>Clade size</th><th>Taxa</th></tr></thead><tbody>{result.discordantClades.map((clade, index) => <tr key={`${clade.betweenSegments.join("-")}-${clade.direction}-${index}`}><td>{clade.betweenSegments.join(" → ")}</td><td>{clade.direction}</td><td>{clade.size}</td><td>{clade.taxa.join(", ")}</td></tr>)}</tbody></table></div></div></details>}
