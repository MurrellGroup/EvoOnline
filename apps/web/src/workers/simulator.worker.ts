...
scope.onmessage = (event: MessageEvent<SimulatorWorkerRequest>): void => {
  ...
  void runSimulator(decodeSimulatorConfig(request.parameters.simulatorConfig), {
    ...(request.replicateIndices === undefined ? {} : { replicateIndices: request.replicateIndices }),
    ...(request.includeDiagnostic === undefined ? {} : { includeDiagnostic: request.includeDiagnostic }),
