...
Every mapped chain also gets a windowed profile-to-chain sequence view directly above Mol*. Its WebLogo-like stacks use raw amino-acid frequency across all input sequences—not information content or entropy normalization—so stack height equals non-gap, unambiguous occupancy and empty height represents missing sequence mass. Explicit traceback indices preserve chain insertions and profile gaps without inflating the browser DOM for long proteins.
