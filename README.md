...
Both model results include an optional reference-map studio after the main posterior table. A user can upload one protein FASTA/plain sequence or one coding-nucleotide reference, which is translated when necessary and globally aligned to the complete amino-acid frequency profile with expected BLOSUM62 scores and affine gaps in a dedicated worker. Unlike the local structure-chain mapper, the global traceback retains profile-only and reference-only insertions, including terminal overhangs.

The pure reference row is drawn above the raw-frequency amino-acid profile with the same normalized vector glyphs. DifFUBAR exposes independent lanes for P(ω·G1>ω·G2), P(ω·G2>ω·G1), P(ω·G1>1), and P(ω·G2>1); FUBAR exposes P(β>α) and P(α>β). Any combination can be shown. Every detection is labeled in a separate collision-free lane using reference coordinates. Profile insertions do not advance the reference number and instead receive spreadsheet-style suffixes (`76A`, `76B`, `76C`, …); reference-only residues remain visible as profile gaps. Threshold, codon window, reference start, horizontal scale, row heights, number size, ticks, guide lines, match highlighting, labels, and colors are editable, and the complete live figure exports directly to SVG.
