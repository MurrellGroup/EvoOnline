...
7. Running DifFUBAR, FUBAR, fixed three-rate BS-REL, FAME, FLAVOR, Glamma, CladeShift, FSART, MosaicSPR, or JEMSPR in dedicated browser workers. Exact parallel WASM is the default for selection models; DifFUBAR and regular FUBAR also retain selectable WebGPU. FSART and MosaicSPR reuse the existing FastTree bioWASM runtime for trees and likelihoods. JEMSPR independently infers its neighbor-joining guides, rooted master, local trees, breakpoints, and coherent event network, then accepts only one fixed global GTR matrix from FastTree for its own linked branch-length likelihood.
...
apps/
  ...
  evo-cli/              Browser-pipeline runner and standalone release entrypoint
packages/
  ...
  phylo-likelihood/     Linked-tree pruning, gradients, branch ML and HMMs
models/
  ...
  simulator/            Sampled coalescent, MG94/SCUFF and recombination simulation
docs/
  ARCHITECTURE.md
  ADDING_A_MODEL.md
  GENETIC_CODES.md
...
The browser uses a CPU-bounded pool of independent FastTree 2.1.11 bioWASM runtimes. Consensus cuts define atomic segments; EvoOnline fits a complete tree to each segment, every adjacent segment pair, every adjacent triplet, and the full alignment. Every resolved full-tree fit is retained independently, including same-topology trees with different fitted branch lengths or Gamma shapes. A shared global GTR model is retained, and an internal Gamma20 likelihood engine scores all sites while freezing each candidate's fitted tree parameters. This moves the combinatorial part into cached emission space. An O(L×trees) reset-HMM uses a beam plus floating add/drop/swap search for the AIC, AICc (default), or BIC full-tree subset, then exactly verifies a bounded finalist set before forward–backward and Viterbi inference. Viterbi-assigned ranges are concatenated to refit trees and the HMM is rerun for a few bounded rounds; full-tree/boundary convergence is recorded but not required.
...
Independently of topology/network search, the alivibe bridge runs FastTree once on the whole alignment and returns only its four GTR frequencies and six exchangeabilities; its tree, branch lengths, CAT rates, support, and likelihood are discarded. That matrix is consumed only after the switching structure is fixed. `packages/phylo-likelihood` then performs a native reversible-GTR eigensystem, custom discrete-Gamma profile, scaled Felsenstein pruning, analytic all-edge gradients, and L-BFGS optimization. Every non-horizontal switching-DAG edge owns one shared length. A displayed branch is the sum of its underlying network-edge path, the instantaneous alternate-parent recombination edge is fixed at zero, and source/destination subdivisions therefore estimate the event's positions without independently refitting daughter trees. A custom forward/backward plus Viterbi pass can refine the genomic mask path and trigger one linked-length refit. Those polished displayed trees now enter the generic fixed-relative codon partition; the lower-level linked-tree objective also remains reusable if a future codon optimizer needs to refit network-edge parameters directly.

The JEMSPR result studio additionally exposes three exact-display views of the SPR relation: a speed-controlled master-to-region animation with continuously interpolated subtree motion, a source-context before/after storyboard for each event, and a mask-state graph whose edges toggle one persistent SPR template. Each current frame or static figure exports as native SVG.

The result studio separates the coherent network from its tree-path relaxation and linked likelihood. It provides an editable genomic tree/event-lane SVG, linked regional trees and two-tree tanglegrams, the compiled switching-DAG SVG, fitted atomic-edge tables and length-bearing Newicks, likelihood/Viterbi regions, event and endpoint tables, exact fixed-network min-marginal boundary gaps, regularization/search audits, event CSV, local-tree and breakpoint TSVs, master Newick, and complete network/result JSON. Fixed-graph and fixed-network inference are exact; rooted-tree column generation and the outer network beam are budgeted and are never labelled globally optimal. See [`models/jemspr`](models/jemspr/README.md).
