...
7. Running DifFUBAR, FUBAR, fixed three-rate BS-REL, FAME, FLAVOR, Glamma, CladeShift, or FSART in dedicated browser workers. Exact parallel WASM is the default for selection models; DifFUBAR and regular FUBAR also retain selectable WebGPU. FSART needs only an alignment and uses parallel typed-array scan workers plus the existing FastTree bioWASM runtime.
...
The result studio provides linked consensus-proposal/interval, triplet topology/HMM, tree-family, topology-loading, Viterbi-run, convergence-audit, and tree-comparison views. It preserves the conservative IC reconstruction and adds two instant post-analysis alternatives over the complete cached tree bank: a user-controlled low-switch/Viterbi-retention mode and a sparse symmetric-Dirichlet variational EM. A separate unrestricted event layer jointly revises an unknown master tree and a piecewise path through a connected graph of executable unrooted SPR moves. Local trees may be arbitrary multi-SPR compositions and one breakpoint may carry a multi-edit script; the UI exposes the master, run track, edit tape, ambiguity, and a candid local/budgeted search certificate. Final HMM runs can still be selected individually, as a rerooted mirrored two-tree tanglegram, or all at once as jointly ordered same-direction trees. This is an independent RDP-inspired, BURT-style, GARD-like approximation—not exact parity with any of those programs. See [`models/fsart`](models/fsart/README.md).
