...
Each chain has a small state machine derived from two checkboxes: `mapped` (shown and colored), `context` (shown neutrally), or `hidden`. Cartoon, ball-and-stick, and molecular-surface flags are stored orthogonally per chain. MolViewSpec union selectors are built independently for those three representations, prevent hidden chains from entering any component, and apply result layers only to mapped-chain residue selectors.
