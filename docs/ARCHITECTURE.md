...
The resulting site-by-topology matrix is immutable during likelihood-HMM model selection. A sticky/reset HMM evaluates candidate topology subsets in O(LK), using a bounded beam over subset size followed by floating add/drop/swap moves under AIC, AICc, or BIC. Exact forward–backward fitting and Viterbi decoding run on the selected subset; assigned ranges are concatenated through `score-fasttree-ranges`, topologies are refitted, and all-site emissions plus subset search repeat for a bounded number of rounds.

FSART also runs a separate, worker-isolated event reconstruction. A canonical labelled unrooted-tree graph enumerator generates every distinct one-SPR neighbour and records both forward and inverse cut/attachment/regraft splits; tests execute every six-taxon neighbour in both directions. Candidate topologies receive exact sitewise Fitch costs. Several data-ranked FastTree seeds launch connected graph searches, while family split distance guides a structurally diverse screen. Successive rounds expand occupied and look-ahead frontier nodes, so topology states are arbitrary compositions rather than mutually exclusive one-SPR alternatives. For a fixed graph, all-pairs shortest paths supply the exact edit count between any two states and an O(LK²) minimum-duration semi-Markov decoder permits a multi-edit transition at one alignment boundary. Path decoding alternates with occupancy-weighted graph-medoid master selection. Exported breakpoint scripts are executable, and shortest-path multiplicity is reported. Since global unrooted-SPR/parsimony search is combinatorial, operational state/screen/round budgets are exposed and the result distinguishes a complete final one-SPR-neighborhood certificate from ordinary budgeted column generation; neither is mislabeled as a global optimum.
