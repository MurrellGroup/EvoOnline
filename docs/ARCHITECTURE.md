...
Each chain has a small state machine derived from two checkboxes: `mapped` (shown and colored), `context` (shown neutrally), or `hidden`. A MolViewSpec union selector prevents hidden chains from entering any representation and applies result layers only to mapped-chain residue selectors. Mapped chains also receive independent, windowed SVG profile-alignment panels. Their glyph mass is `within-valid-column frequency × valid codons / total sequences`; stacks therefore encode raw occupancy rather than sequence-logo information content. Mol* is pinned but loaded from a CDN only on demand; its declarative state keeps controls limited to chain modes, color, cartoon, atoms, and surface. Deleting the feature directory and the one panel import from each result renderer removes the capability without touching either numerical model.
