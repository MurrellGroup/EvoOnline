...
MosaicSPR is separately registered and owns a separate worker, parameter manifest, result contract, and renderer. It imports FSART's proposal-level scanner, uncertainty refinement, consensus aggregation, and window builders without invoking an FSART analysis or modifying its result. Optional consensus regions plus the overlapping safety bank request local FastTree fits through the same alivibe bridge. Those trees are topology seeds only.

The MosaicSPR worker parses labelled Newick into a canonical unrooted graph, suppresses the artificial degree-two root, and enumerates every distinct one-SPR neighbour with an executable directed cut/regraft record plus its inverse. Candidate topologies receive exact sitewise Fitch costs. Multiple starts, successive neighbor layers, and look-ahead through neutral intermediates permit unrestricted compositions rather than one mutually exclusive SPR category per site. Within each explored connected graph, all-pairs shortest paths provide edit distances and scripts; an exact minimum-duration semi-Markov decoder permits a multi-edit transition at one genomic boundary, while an occupancy-weighted graph-medoid update revises the unknown master. The explicit graph/path result is exact; outer topology-space column generation is budgeted and never labelled globally optimal.

MosaicSPR's linked-tree renderer consumes only the reconstructed runs and the trees stored on their occupied graph states. Thus every displayed regional phylogeny is implied by replaying the event tape. The same display-only reroot/child-order optimizer used by FSART minimizes matching-tip crossings for pair and all-region comparisons, but the two methods do not share analysis state or UI panels.

JEMSPR is a third independent recombination package and worker. Its dependency list contains only the generic domain and model SDK packages: it imports no FSART/MosaicSPR code, never calls the alivibe/FastTree bridge, accepts no supplied tree, and consumes no breakpoint proposal artifact. It builds ambiguity-aware genomic cells directly from the alignment, infers whole-alignment and data-independent dyadic-window neighbor-joining guides internally, enumerates several root placements, and scores canonical rooted trees with exact Fitch or Sankoff recursion.

JEMSPR's path relaxation maintains a verified one-rooted-SPR graph. It enumerates candidate masters and solves the complete genomic path exactly for each one, including arbitrary graph-distance changes at a boundary and master-distance span. Forward/backward arrays price an omitted one-rSPR topology over every possible single contiguous interval in linear genomic time after boundary transforms. Occupied states, strict interval improvers, near improvers, and moves toward independent multiscale guides drive budgeted graph expansion. Complete neighborhoods remain available for small-tree exact tests; production search streams a deterministic, structurally distributed candidate budget and retains only the best proxy columns, preventing the transient O(n³)-like tree-object volume of collecting every rooted-SPR neighbor before screening. Every retained edge is still an executable rooted-SPR move. The fixed graph optimum is exact; the outer tree universe is explicitly resource bounded.

The second stage begins from the top distinct candidate masters across every independently expanded root start, then compiles residual rooted-SPR moves into binary switching DAGs rather than calling local transition scripts an event history. One compilation inserts a recipient reticulation and donor-edge subdivision with ordered background/alternate parents, verifies all prior zero-bit displays, realizes the requested alternate display, and exhaustively confirms that every mask retains all labeled leaves. Each candidate network is scored by an exact active-mask DP over every mask up to the selected overlap cap. Maximal one-runs become persistent event occurrences; crossing and nested intervals require no stack discipline. A donor–recipient equality contraction plus strict-ancestry DAG test is applied as a hard lazy cut before an infeasible child can enter the beam. The outer network beam is budgeted, while every fixed-network score and decoded mask history is exact. Its renderer exports separate genomic-event, linked-tree, and switching-DAG SVGs plus event CSV, local-tree/breakpoint TSV, master Newick, and network JSON.
