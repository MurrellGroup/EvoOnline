...
Global-Gamma uses a different hierarchy on the same topology. Its outer α category is shared across every edge of one site; inside that α-specific tree prune, each edge independently applies the globally weighted ω transition mixture. Alpha categories are collapsed only after the complete site likelihood is available. The final all-message kernel retains low-ω and positive-tail pieces around every edge. Replacing the tail by its ω=1 transform gives the exact one-edge capped likelihood, while contracting only the retained tail gives P(ω>1 | data) for each branch/site cell. Alpha-weighted local likelihoods are integrated before either value leaves WASM. A separate all-capped prune supplies the site contrast. The branch activation integral consumes the same local capped/uncapped ratios, so exact branch truncation is always available without an additional per-branch prune.
